...
# We need to updated the current okteto context because as we are using the kubeconfig generated by okteto, it uses internally
# the current context to authenticate with the cluster
okteto ctx use "${OKTETO_URL}" --token "$token"
